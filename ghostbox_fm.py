...
class SpiritBoxFM:
    ...
    def run(self, get_hold_time_func, get_volume_func): # Dodano funkcję pobierania czasu
        ...
        try:
            ...
            while self._looping: # Sprawdzaj flagę na początku każdej iteracji
                # Pobierz aktualny czas 'hold' z GUI
                ...
                # Odczytaj próbki - ilość zależy od hold_time
                ...
                # Następna częstotliwość (jeśli nadal pętla działa)
                if self._looping:
                    current_freq_iter += FM_STEP
                    if current_freq_iter > FM_END_FREQ:
                        current_freq_iter = FM_START_FREQ
                # Usunięto time.sleep() - prędkość jest teraz kontrolowana przez read_samples()
...
def start_scan_gui():
    ...
    # Przekaż funkcje do pobierania wartości z GUI
    scan_thread_gui = threading.Thread(target=sb.run, args=(get_current_hold_time, get_current_volume), daemon=True)
...
window = tk.Tk()
window.title("Ghost Box Pi - FM (vFinal)")
...
start_button = tk.Button(controls_frame, text="START", command=start_scan_gui, font=label_font, bg='green', fg='white', width=10, height=1, state=tk.NORMAL)
start_button.pack(side=tk.LEFT, padx=10)

stop_button = tk.Button(controls_frame, text="STOP", command=stop_scan_gui, font=label_font, bg='red', fg='white', width=10, height=1, state=tk.DISABLED)
