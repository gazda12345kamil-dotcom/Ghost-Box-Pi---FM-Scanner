#!/usr/bin/env python3
# =============================================================================
# Ghost Box Pi - Wersja PRO v5.1 (FINAL + REC/S-Meter/Save + DSP+ / BugFix)
#
# Funkcje:
...
# - Nagrywanie audio do .wav (Przycisk REC)
# - Sygnałomierz (S-Meter)
# - Zapisywanie/Wczytywanie ustawień (config.json)
# - Filtr audio (300-3400Hz) dla lepszej czytelności głosu
# - Filtrowanie stanowe (IIR) eliminujące "kliki" między blokami
# - Automatyczna Kontrola Wzmocnienia (AGC) zamiast prostej normalizacji
#
# POPRAWKI (v5.1):
# - Poprawka błędu przy starcie nagrywania (przycisk REC pozostawał w złym stanie)
# - Poprawka zamykania aplikacji (gwarantowane zwolnienie zasobu SDR)
# =============================================================================
...
class SpiritBoxMultiBand:
    def __init__(self, msg_queue):
        ...
        # === Ustawienia AGC i Filtra Audio (DSP+) ===
        
        # --- Ustawienia AGC ---
        self.log("Inicjalizacja AGC...")
        self.agc_gain = 1.0           
        self.target_rms = 0.1         
        self.agc_attack_rate = 0.002  
        self.agc_decay_rate = 0.001   
        
        # --- Filtr Audio (Post-Demodulacja) ---
        self.log("Tworzenie filtra audio (300-3400 Hz)...")
        nyquist = AUDIO_SAMPLE_RATE / 2
        low_cutoff = 300 / nyquist
        high_cutoff = 3400 / nyquist
        self.audio_b, self.audio_a = signal.butter(5, [low_cutoff, high_cutoff], btype='bandpass')
        
        self.log("Tworzenie stanu filtra audio...")
        self.audio_filter_state = signal.lfilter_zi(self.audio_b, self.audio_a)
    ...
    def close_sdr(self):
        # ZMIANA: Dodano logowanie i sprawdzanie, czy sdr nie jest None
        if self.sdr:
            try: 
                self.sdr.close()
                self.sdr = None
            ...
            self.log("SDR był już zamknięty.")


    # === Konwersja mocy na S-Meter ===
    ...
    def run(self, get_hold_time_func, get_volume_func, get_squelch_level_func, get_random_mode_func, get_active_bands_func):
        ...
        try:
            ...
            while self._looping:
                ...
                # --- 6. SQUELCH, S-METER i DEMODULACJA ---
                ...
                # --- 6b. FILTRACJA AUDIO (STANOWA) ---
                if audio_samples is not None and audio_samples.size > 0:
                    audio_samples, self.audio_filter_state = signal.lfilter(
                        self.audio_b, self.audio_a, 
                        audio_samples, 
                ...
                # --- 7. Odtwarzanie Audio (z AGC) i NAGRYWANIE ---
                if audio_samples is not None and audio_samples.size > 0 and self._looping:
                    
                    # --- Logika AGC ---
                    current_rms = np.sqrt(np.mean(audio_samples**2))
                    required_gain = self.target_rms / (current_rms + 1e-9)
                    
                    if required_gain > self.agc_gain:
                        self.agc_gain = (1.0 - self.agc_attack_rate) * self.agc_gain + self.agc_attack_rate * required_gain
                    else:
                        self.agc_gain = (1.0 - self.agc_decay_rate) * self.agc_gain + self.agc_decay_rate * required_gain
                    
                    self.agc_gain = np.clip(self.agc_gain, 0.1, 100.0)
                    audio_agc = audio_samples * self.agc_gain
                    
                    volume_level = get_volume_func() / 100.0
                    audio_final = audio_agc * volume_level
                    
                    audio_final = np.clip(audio_final, -1.0, 1.0).astype(np.float32)

                    try:
                        if not self._looping: break
                        if audio_stream: audio_stream.write(audio_final)
                        
                        if is_recording.is_set():
                            rec_queue.put(audio_final.copy())
                            
                    except Exception as audio_err:
                        self.log(f"Błąd zapisu audio: {audio_err}"); time.sleep(0.05)
                
                elif audio_samples is None and self._looping:
                    pass
            ...
            audio_stream = None
            self.close_sdr() # <-- To jest kluczowe
            self._looping = False
    ...
    def close(self): 
        # ZMIANA: Modyfikacja, by 'close' wywoływało obie funkcje
        self.stop()
        self.close_sdr()
...
band_check_vars = {}
start_button, stop_button, rec_button = None, None, None 
...
# =================================================================
# === Ustawienia, Nagrywanie, S-Meter (Funkcje Pomocnicze) ===
# =================================================================
...
def rec_writer_thread():
    ...
    global is_recording, rec_queue, msg_queue
    ...
    try:
        ...
        with sf.SoundFile(filename, mode='w', samplerate=AUDIO_SAMPLE_RATE, channels=1, subtype='PCM_16') as file:
            ...
            # Dopiero po pomyślnym otwarciu pliku logujemy sukces
        ...
        # === NOWOŚĆ (POPRAWKA BŁĘDU 2) ===
        # Wyślij sygnał błędu do GUI, aby zresetować przycisk
        try:
            msg_queue.put(f"[REC_ERROR] {e}", block=False)
        except queue.Full:
            pass # Trudno, GUI się nie dowie
...
def toggle_recording():
    ...
    if is_recording.is_set():
        ...
        is_recording.clear()
        rec_button.configure(text="REC 🔴", fg_color="#CC0000", hover_color="#FF0000") 
        log_message_gui("Zatrzymywanie nagrywania... Opróżniam bufor.")
    else:
        # Rozpoczynanie nagrywania
        is_recording.set()
        # Wyczyść starą kolejkę
        ...
        rec_thread.start()
        # ZMIANA: Przycisk zmienia się OD RAZU, ale błąd (obsłużony w check_msg_queue) go zresetuje
        rec_button.configure(text="STOP ⏹", fg_color="red", hover_color="#AA0000") 
...
def start_scan_gui():
    ...
    rec_button.configure(state=ctk.NORMAL) 
...
def stop_scan_gui():
    ...
    sb.stop() # Wyślij sygnał stopu do wątku
...
def check_msg_queue():
    ...
    try:
        while True:
            ...
            elif message.startswith("[SMETER]"): 
                val = float(message.split(' ')[1])
                update_smeter_bar(val)
            
            # === NOWOŚĆ (POPRAWKA BŁĘDU 2) ===
            elif message.startswith("[REC_ERROR]"):
                log_message_gui(f"Błąd REC: {message.split(' ', 1)[1]}")
                # Sprawdź, czy GUI myśli, że nagrywa
                if is_recording.is_set():
                    toggle_recording() # Zresetuj stan (zmieni flagę i przycisk)
...
def update_smeter_bar(value): 
    try:
        if window.winfo_exists(): smeter_bar.set(value)
    except Exception: pass

def on_closing():
    # === ZMIANA (POPRAWKA BŁĘDU 1) ===
    # Ulepszona, bardziej niezawodna funkcja zamykania
    global sb, scan_thread_gui, is_recording, rec_thread
    log_message_gui("Zamykanie aplikacji...")
    
    save_settings() # Zapisz ustawienia
    
    # 1. Zatrzymaj nagrywanie (daj mu 1s na zapis bufora)
    if is_recording.is_set():
        is_recording.clear()
        if rec_thread and rec_thread.is_alive():
            log_message_gui("Czekam na zakończenie zapisu...")
            rec_thread.join(timeout=1.0) # Akceptujemy 1s freez
            
    # 2. Zatrzymaj wątek SDR
    if sb.looping:
        sb.stop() # Sygnalizuj zatrzymanie
        if scan_thread_gui and scan_thread_gui.is_alive():
             log_message_gui("Czekam na zamknięcie wątku SDR...")
             scan_thread_gui.join(timeout=0.5) # Czekaj na 'finally'
    
    # 3. Ręcznie zamknij zasób SDR (gwarancja zwolnienia)
    # Ta funkcja jest bezpieczna do wielokrotnego wywołania.
    # Uwalnia zasób, nawet jeśli .join() powyżej się nie powiódł.
    try:
        log_message_gui("Wymuszanie zamknięcia zasobu SDR...")
        sb.close_sdr()
        log_message_gui("Zasób SDR zwolniony.")
    except Exception as e:
        log_message_gui(f"Problem przy zamykaniu SDR: {e}")
...
def show_help():
    help_text = """Ghost Box Pi - Wersja PRO v5.1 (DSP+)
...
NOWOŚCI (DSP+):
• Jakość dźwięku została znacznie poprawiona przez filtr mowy (300-3400Hz) oraz automatyczną kontrolę wzmocnienia (AGC).
"""
    messagebox.showinfo("Pomoc - Ghost Box Pi PRO v5.1", help_text)
...
window = ctk.CTk()
window.title("Ghost Box Pi - PRO v5.1 (REC/S-Meter/DSP+)")
...
sliders_frame.grid(row=3, column=0, padx=10, pady=5, sticky="ew") 
...
squelch_frame.grid(row=4, column=0, padx=10, pady=5, sticky="ew") 
...
controls_frame.grid(row=5, column=0, padx=10, pady=10, sticky="ew") 
...
log_text.grid(row=6, column=0, padx=10, pady=(5, 0), sticky="nsew") 
